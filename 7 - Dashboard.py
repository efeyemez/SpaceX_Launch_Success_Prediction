...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fil_df1 = df[df["Launch Site"]==entered_site]
        fil_df2 = fil_df1.groupby(["Launch Site", "class"]).size().reset_index(name='class count')
        
        fig = px.pie(fil_df2, values="class count",
        names='class', 
...
def get_scatter_chart(entered_site, entered_payload):
    ...
    range = spacex_df[spacex_df["Payload Mass (kg)"].between(low, high)]

    if entered_site == 'ALL':
        fig = px.scatter(range, x="Payload Mass (kg)", y="class",
                        color="Booster Version Category",
                        title="Correlation between Payload and Success for all Sites")
        return fig

    else:
        fil_range = range[range["Launch Site"]==entered_site]


        fig = px.scatter(fil_range, x="Payload Mass (kg)", y="class",
                        color="Booster Version Category",
